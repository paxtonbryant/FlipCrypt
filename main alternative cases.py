...
#Handles case where Encrypt column named "ENCRPT"
		if re.search("Name \"[A-Za-z_].*_ENCRPT\"", line):
			GenEncryptFlag = True
			DerivationFlag = False
# Handles case where Encrypt column named "ENCR"
		if re.search("Name \"[A-Za-z_].*_ENCR\"", line):
			GenEncryptFlag = True
			DerivationFlag = False
# Handles case where Encrypt column named "ENCRYPT_#"
		if re.search("Name \"[A-Za-z_].*_ENCRYPT*\"", line):
			GenEncryptFlag = True
			DerivationFlag = False
# Handles case where Encrypt column named "ENCRYPTED"
		if re.search("Name \"[A-Za-z_].*_ENCRYPTED*\"", line):
			GenEncryptFlag = True
			DerivationFlag = False
# Handles case where Encrypt column named "ENCRPYT"
		if re.search("Name \"[A-Za-z_].*_ENCRPYT*\"", line):
			GenEncryptFlag = True
			DerivationFlag = False
